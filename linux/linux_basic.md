...
| ---- | ------------ | ------------------------------------------------ |
| grep | 过滤           | 过滤速度最快                                           |
| sed  | 替换，修改文件内容，取行 | <p>如果要进行替换/修改文件内功<br>取出某个范围的内容（从10：00-12:00）</p> |
| Awk  | 取列，统计计算      | <p>取列<br>对比，比较<br>统计，计算(awk数组)</p>               |
...
| -n | 显示行号                |                                                          |
| -i | 忽略大小写               |                                                          |
| -w | 精确匹配                | <p>echo a ab | grep -w a<br>echo a ab | grep '\ba\b'</p> |
| -o | 只显示匹配到的内容           |                                                          |
...
| -- | ------------------ | ----------------- | ------------------ |
| c  | 替代(加g全局替换)         | s/a/b/g           |                    |
| a  | append 向指定行或每一行后追加 | sed '1a 996' file | sed '1a\ 996' file |
| i  | 插入行，在每一行前          |                   |                    |
...
| 模式      | 含义       | 应用场景                                                |
| ------- | -------- | --------------------------------------------------- |
| BEGIN{} | 在读取文件前执行 | <p>1) 进行简单统计计算<br>2) 添加表头<br>3) 用于定义awk变量</p>       |
| END{}   | 读取后执行    | <p>1) awk进行统计，先计算，最后END输出结果<br>2) 使用数组，用来输出数组结果</p> |
...
| 批量输出数组内容 | <p>for( i in array )<br>print i #下标<br>print array[i] #内容</p> | <p>For i in ${array[*]}<br>do<br>echo ${i}<br>done</p> |

删除数组元素：delete array\_name\[index]
